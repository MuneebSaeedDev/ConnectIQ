/**
 * Data loader for the Destination List Screen (SCR-045, node 115:26755,
 * Figma page "Page 1", frame "Destination List Screen").
 *
 * MOCK BOUNDARY: MOD-006 (Destinations & Connectors) is still `PLANNED`
 * with no backend deployed — no `Destination`, `ConnectionTestResult`, or
 * `SourceHealthRecord` endpoint exists yet (see docs/modules/module-plan.md).
 * `getDestinations` always attempts a real GET first and only falls back to
 ...
 * header (breadcrumb Data Management › Destinations, title "Destinations",
 * subtitle, Refresh / Export Destinations / Import Configuration / + Add
 * Destination actions), the 6 KPI cards (Total Destinations 47 · 12 source
 ...
 * Environments / All Owners / All Owners / Last Checked, Columns, Clear
 * Filters), the bulk-action bar ("3 destinations selected" · Test
 ...
 * modified), the "Showing 14 of 47 destinations" pager, and the right-hand
...
const MOCK_TITLE =
  'MOD-006 (Destinations & Connectors) has no backend deployed yet — showing sample data, not live records.';
 ...
 */
export const MOCK_DESTINATION_LIST = {
  updatedAt: 'Jan 15, 2025',
  organizationName: 'Acme Corporation',
  kpis: [
    { key: 'total', label: 'Total Destinations', value: '47', hint: '12 source types', tone: 'default' },
 ...
 */
export async function getDestinations(orgId, filters = {}) {
  ...
  try {
    const res = await apiFetch(
      `/organizations/${encodeURIComponent(orgId)}/destinations${query}`,
    );
    if (!res.ok || !res.headers.get('content-type')?.includes('application/json')) {
      throw new DestinationListError('Unable to load destinations right now.');
    }
    const data = await readJson(res);
    if (!data || !Array.isArray(data.rows)) {
      throw new DestinationListError('Received an unexpected response from the server.');
    }
    return { ...data, mocked: false };
  } catch {
    return { ...MOCK_DESTINATION_LIST, mocked: true };
  }
}
