...
export default function DuplicateNodeModal({
  ...
  const [nodeName, setNodeName] = useState(`${currentNodeName || 'snowflake_warehouse_load'}_copy`);
      ...
      <div className="bg-white rounded-lg border border-slate-200 shadow-xl max-w-md w-full overflow-hidden">
        <div className="px-5 py-3.5 bg-slate-50 border-b border-slate-200 flex items-center justify-between">
          <div className="flex items-center gap-2 text-slate-800 font-bold text-xs">
            ...
            <h3 id="duplicate-node-title">Duplicate Destination Node</h3>
          </div>
          <button
            type="button"
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="space-y-1.5">
            ...
            <input
              ...
              className="w-full px-3 py-1.5 font-mono text-xs border border-slate-300 rounded bg-white focus:outline-hidden focus:ring-2 focus:ring-emerald-500"
              autoFocus
            />
            <p className="text-[11px] text-slate-500">
              All target table settings, schema projections, write modes, and buffer configurations will be cloned.
            </p>
          </div>
          ...
          <div className="flex items-center justify-end gap-2 pt-2 border-t border-slate-100">
            ...
            <button
              type="submit"
              className="px-3.5 py-1.5 text-xs font-semibold text-white bg-emerald-600 hover:bg-emerald-700 rounded transition"
            >
              Create Duplicate
            </button>
          </div>
