...
export default function MergeNodeHeader({
  form = {},
  isDirty,
  unsavedChangesCount,
  onReset,
    ...
    <header className="bg-white border-b border-slate-200 sticky top-0 z-20 shadow-xs">
      <div className="px-6 py-3">
        ...
        <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-slate-500 mb-1.5 font-medium">
          ...
          <ChevronRight className="size-3 text-slate-400" />
          <span className="hover:text-slate-800 transition cursor-pointer">Nodes</span>
          <ChevronRight className="size-3 text-slate-400" />
          <span className="text-slate-900 font-semibold">Merge Node Configuration</span>
        </nav>
        ...
        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-0.5">
            <div className="flex items-center gap-3">
              <div className="size-7 rounded-md bg-indigo-50 border border-indigo-200 flex items-center justify-center text-indigo-600 shrink-0">
                <GitMerge className="size-4" />
              </div>
              <h1 className="text-lg font-bold text-slate-900 tracking-tight">
                {form.nodeName || 'Merge Node Configuration'}
              </h1>
              {/* Status Pill */}
            ...
            <p className="text-xs text-slate-500">
              Configure multi-stream join criteria, field conflict resolutions, and in-memory hash buffer allocation.
            </p>
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            <button
              ...
              title="Duplicate this merge node"
            >
            ...
            <button
              ...
              onClick={onCancel}
              className="inline-flex items-center gap-1.5 px-3 py-1.5 text-xs font-medium text-slate-700 bg-white hover:bg-slate-50 border border-slate-300 rounded-md transition shadow-2xs"
              title="Cancel and return to pipeline canvas"
            >
            ...
            <button
              ...
              title="Validate join conditions and schema alignment"
            >
              {isValidating ? (
