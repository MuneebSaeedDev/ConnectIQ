...
export function useMappingNodeConfig(nodeId = 'map_node_0041', pipelineId = 'customer-etl-v2') {
  ...
  const computedSummary = useMemo(() => {
    ...
    const mappedTargetSet = new Set(
      form.mappings.filter((m) => m.targetField && m.targetField !== '—').map((m) => m.targetField)
    );

    const requiredMappedCount = (form.targetSchema?.fields || [])
      .filter((f) => f.required && mappedTargetSet.has(f.name)).length;

    const directCount = form.mappings.filter((m) => m.mappingType === 'Direct').length;
    const transformCount = form.mappings.filter((m) => ['Expression', 'Rename', 'Conditional', 'Lookup'].includes(m.mappingType)).length;
    const castCount = form.mappings.filter((m) => m.mappingType === 'Cast').length;

    const passCount = form.mappings.filter((m) => m.validationStatus === 'pass').length;
    const warnCount = form.mappings.filter((m) => m.validationStatus === 'warning').length;
    const errCount = form.mappings.filter((m) => m.validationStatus === 'error' || m.status === 'missing').length;

    const completion = Math.round((mappedTargetSet.size / totalTarget) * 100);
    ...
    return {
      ...
      mappingStatus: {
        totalMappings: form.mappings.length,
        complete: passCount,
