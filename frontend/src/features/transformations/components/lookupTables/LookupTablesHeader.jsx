...
import { Plus, Download, Upload, RefreshCw, Table } from 'lucide-react';

export default function LookupTablesHeader({
  onCreateTable,
  onImportTables,
  onExportTables,
  onRefresh,
      ...
      <div>
        <div className="flex items-center gap-2">
          ...
          <span className="inline-flex items-center px-2 py-0.5 rounded text-xs font-medium bg-blue-50 text-blue-700 border border-blue-200">
             <Table className="w-3 h-3 mr-1 text-blue-600" />
            Reference Data
          </span>
        </div>
        <p className="text-xs sm:text-sm text-slate-500 mt-1 max-w-3xl">
          Create, configure, manage, test, and reuse lookup tables for reference-based data enrichment within ETL pipelines.
        </p>
      </div>
