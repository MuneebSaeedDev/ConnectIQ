    ...
    <header className="bg-white border-b border-slate-200 sticky top-0 z-20 shadow-2xs">
      <div className="max-w-[1920px] mx-auto px-6 py-3">
        <nav aria-label="Breadcrumb" className="flex items-center gap-1.5 text-xs text-slate-500 mb-1.5 font-medium">
          ...
          <ChevronRight className="size-3 text-slate-400" />
          <span className="text-slate-900 font-semibold">Mapping Node Configuration</span>
        </nav>

        <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
          <div className="space-y-0.5">
            <div className="flex items-center gap-3">
              <div className="size-8 rounded-lg bg-indigo-50 border border-indigo-200 flex items-center justify-center text-indigo-600 shrink-0">
                <GitMerge className="size-4" />
              </div>
              <h1 className="text-lg font-bold text-slate-900 tracking-tight">
                {nodeName || 'Mapping Node Configuration'}
              </h1>
            ...
            <p className="text-xs text-slate-500">
              Map fields between input schemas and target schemas, apply transformations, and validate mappings.
            </p>
          </div>
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            <button
              type="button"
              onClick={(e) => { e.preventDefault(); onValidate(); }}
