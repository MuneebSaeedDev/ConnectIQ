...
import { TypeBadge, ConversionDirection } from './TypeBadge';
import { AlertCircle, Plus, Copy, Power, PowerOff, Trash2, Filter } from 'lucide-react';

export const TypeConversionRulesTable = ({
  data = [],
  pagination,
  filters,
  isLoading,
  ...
  if (isLoading) {
    return (
      <div className="p-8 text-center bg-white">
        ...
        <p className="mt-2 text-sm text-[#64748b]">Loading conversion rules...</p>
      </div>
    );
  }
  ...
  if (isError) {
    return (
      <div className="p-8 text-center bg-white">
        ...
        <h3 className="text-sm font-semibold text-[#0f172a]">Failed to load conversion rules</h3>
  ...
  if (!data || data.length === 0) {
    return (
      <div className="p-12 text-center bg-white">
        ...
        <h3 className="text-base font-semibold text-[#0f172a]">No conversion rules found</h3>
        <p className="text-sm text-[#64748b] max-w-md mx-auto mt-1">
          {filters.search || (filters.sourceType && filters.sourceType !== 'All') || (filters.targetType && filters.targetType !== 'All')
            ? 'No rules match your current filter criteria. Try adjusting or clearing the filters.'
            : 'Get started by creating your first type conversion rule to standardize and cast data in ETL pipelines.'}
        </p>
      </div>
    );
  }
        ...
        <table className="min-w-full divide-y divide-[#e2e8f0] text-left text-sm">
          <thead className="bg-[#f8fafc] text-xs font-semibold text-[#475569] uppercase tracking-wider">
            <tr>
              ...
              <th scope="col" className="px-6 py-3">Conversion Direction</th>
              <th scope="col" className="px-6 py-3">Target Fields</th>
              <th scope="col" className="px-6 py-3">Method & Strategy</th>
            ...
            {data.map((rule) => {
              ...
              return (
                <tr key={rule.id} className="hover:bg-[#f8fafc] transition-colors">
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <ConversionDirection sourceType={rule.sourceType} targetType={rule.targetType} />
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap">
                    <div className="flex flex-col">
                      <span className="text-xs font-medium text-[#0f172a]">{rule.conversionMethod}</span>
                      <span className="text-[11px] text-[#64748b] capitalize mt-0.5" title={rule.strategy.replace('_', ' ')}>
                        Strategy: {rule.strategy.replace('_', ' ')}
                      </span>
                    </div>
                  </td>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-right text-xs font-medium">
                    <div className="flex items-center justify-end gap-2">
                      <button
                        ...
                        className="p-1.5 text-[#64748b] hover:text-[#0ea5e9] hover:bg-[#f1f5f9] rounded-md transition-colors"
                        title="Edit Rule"
                      >
                        Edit
                      </button>
